...
void updateRetour(int i, int j) {
	instructions[i].val2 = j;
}

void addFonction(char *fonction){
	if(compteur_fonction <1028){
		strcpy(fonctions[compteur_fonction].nom, fonction);
...
int ajoutInstruWhile(){
 if (compteur_instru < 4096){
   strcpy(instructions[compteur_instru].instru , "JMP");
...
int adrFonct(char * nom){
	...
	printf("::::1 %d\n", compteurInter);
  	while (compteurInter >= 0 && (strcmp(fonctions[compteurInter].nom, nom)!=0)) {
	   printf("::::2 %d    %s\n", compteurInter, fonctions[compteurInter].nom);
  	   compteurInter -- ;
	 }
	 if (compteurInter == -1) {
