...
int ajoutElement(char *id, char *type,int init ,int profondeur){
  if (compteur < 1024){
    strcpy(symboles[compteur].id, id) ;
    strcpy(symboles[compteur].type, type) ;
...
char * retourType(char * symbole){
	return symboles[rechercheElement(symbole)].type;
}
int retourProfondeur(char * symbole){
	return symboles[rechercheElement(symbole)].profondeur;
}
int rechercheElement(char * id){
  int compteurInter = compteur - 1; 

  while (compteurInter >= 0 && strcmp(symboles[compteurInter].id, id)) {
    compteurInter -- ;
  }
  if (compteurInter == -1) {
